function [y4] = superbee(deltaT,tempo,aux,cinj,cini,newVet,oldVet,divEspaco,vetorEspaco,deltaTMAX,C,thetaVet)
tic
clear
clc
entradaDados
...
while aux < tempo 
    ...
    for j = 2:divEspaco-1
       while g < length(oldVet)
        ...
        newVet(j) = oldVet(j) - (C)*(oldVet(j) - oldVet(j - 1)) -(C)/(2) *(1-C)*((psiSuperbee(thetaVet(j+1)))*(oldVet(j+1)-oldVet(j))-(psiSuperbee(thetaVet(j-1)))*(oldVet(j)-oldVet(j-1)))
